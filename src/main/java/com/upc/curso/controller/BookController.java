...
//@CrossOrigin(origins = {"https://main--bright-ganache-a3a851.netlify.app"})
@RestController
@RequestMapping("/api")
public class BookController {
    ...
    @GetMapping("/books/{id_author}")
    public ResponseEntity<List<BookDTO>> list(@PathVariable Long id_author){
        List<Book> list;
        List<BookDTO> listDto;
        try{
            list = bookNegocio.listBooksAuthor(id_author);
            listDto = convertToLisDto(list);
        }catch (Exception e){
            throw new  ResponseStatusException(HttpStatus.NOT_FOUND, "No se pudo listar");
        }
    ...
    @GetMapping("/quantityBooks/{id}")
    public int quantityBooks(@PathVariable(value = "id" ) Long id){
        return bookNegocio.quantityBooks(id);
    }
